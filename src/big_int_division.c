...
static void		divide_out_estimated_quotient(t_big_int *dividend, t_big_int *divisor, uint32_t quotient)
{
	t_div_support	div_sup;
	...
	do
	{
		div_sup.product = (uint64_t)(*divisor_cur) * (uint64_t)quotient + div_sup.carry;
		div_sup.carry = div_sup.product >> 32;
		div_sup.difference = (uint64_t)(*dividend_cur) - (div_sup.product & 0xffffffff) - div_sup.borrow;
...
static void		substract_divisor(t_big_int *dividend, t_big_int *divisor)
{
	uint32_t	*final_divisor_block;
	uint32_t	*divisor_cur;
	uint32_t	*dividend_cur;
	...
	do
	{
		difference = (uint64_t)(*dividend_cur) - (uint64_t)(*divisor_cur) - borrow;
